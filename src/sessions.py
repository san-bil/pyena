from sbpy_utils.core.key_val import kvg, kv_get, kv_set_recurse, kv_get_recurse, kv_set, kv_join
...
from pyena import find_free_hyena_machines, create_hyena_worker_pool,get_next_hyena_host
from core import get_pyena_dir, get_pyena_dep_repos, submit_to_condor, set_global_job_root_dir
import os
import time
import warnings
...
#%                        we'll try to find one.
    ...
    #volatile_src_paths = list(set(volatile_src_paths+[get_pyena_dir()]))
    ...
    session_options['python_path_additions']=':'.join([os.path.join(volatile_src_task_path,os.path.basename(x)) for x in volatile_src_paths])
...
def submit_to_condor_session(session_object,  worker_task, worker_args, job_tags={}, options={}):
    ...
    if(use_hyena):
        ...
        hyena_host_jobs = kv_get_recurse({hyena_host,'host_slots_used'},hyena_pool,0);
    ...
    data_path = os.path.join(task_dir,'task_data.p');
    if(is_remote_submit_host):
        ssh_key=kv_get('ssh_key',session_options,get_default_ssh_key());
        mkdir_p2(task_dir,submit_host,ssh_key);
        save_and_rsync(submit_host,data_path,{'worker_task':worker_task,'worker_args':worker_args,'src_paths':src_paths,'task_dir':task_dir},ssh_key);
    else:
        mkdir_p2(task_dir);
